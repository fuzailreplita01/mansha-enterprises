...
export function GenerativeArtScene() {
  ...
  useEffect(() => {
    ...
    // Safely parse CSS brand variables for Three.js (Electric Yellow)
    let brandColorStr = '#FAFF69';
    if (typeof window !== 'undefined') {
      const brandVar = getComputedStyle(document.documentElement).getPropertyValue('--brand').trim();
      if (brandVar) {
        brandColorStr = brandVar;
      }
    }
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        color: { value: new THREE.Color(brandColorStr) },
      },
      vertexShader: `
        uniform float time;
...
export function AnomalousMatterHero({
  ...
  return (
    <section
      role="banner"
      className="relative w-full h-screen bg-[var(--background)] text-[var(--foreground)] overflow-hidden"
    >
      <Suspense fallback={<div className="w-full h-full bg-[var(--background)]" />}>
        <GenerativeArtScene />
      </Suspense>

      <div className="absolute inset-0 bg-gradient-to-t from-[var(--background)] via-[var(--background)]/70 to-transparent z-10" />

      <div className="relative z-20 flex flex-col items-center justify-end h-full pb-20 md:pb-32 text-center">
        <div className="max-w-3xl px-4">
          <h1 className="text-sm font-mono tracking-widest text-brand/80 uppercase">
            {title}
          </h1>
